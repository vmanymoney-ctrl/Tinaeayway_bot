...
Q1_OPTS = [
    ...
    ("💆 Потрібно вже нарешті зайнятися\nсобою",          "B"),
    ("💰 Треба знайти спосіб\nбільше заробляти",          "I"),
    ("🌀 Я ніби заплуталася",                             "E"),
    ("🕊 Просто хочу, щоб стало легше",                   "E"),
]

Q2_OPTS = [
    ("⏳ Постійно відкладаю початок",                     "E"),
    ("🔄 Починаю, але швидко здаюся",                     "E"),
    ("❓ Не знаю, за що братися спочатку",                "E"),
    ("😶 Наче все роблю, але нічого\nне змінюється",      "E"),
]
...
Q3_OPTS = [
    ...
    ("💸 Гроші",                                          "I"),
    ("🪞 Моє здоров'я або зовнішній вигляд",              "B"),
...
WELCOME_TEXT = (
    "Привіт ❤️\n\n"
    "Якщо ти зараз тут — швидше за все, відчуваєш, що хочеться щось змінити.\n\n"
    "Я Валентина, і дуже рада, що ти тут.\n\n"
    "Я знаю, що коли одночасно хочеться більше сил, краще виглядати, "
    "більше заробляти і просто перестати жити в постійному поспіху — "
    "дуже легко розгубитися й не зробити нічого.\n\n"
    "Тому сьогодні я не буду говорити тобі, що тобі «потрібно».\n\n"
    "Я поставлю кілька запитань, подивлюся на твою ситуацію в цілому "
    "і в кінці скажу, <b>з чого я б почала на твоєму місці.</b> "
    "Не як експерт чи коуч, а як подруга за чашечкою кави. ☕"
)

TEXT_BETWEEN_Q2_Q3 = (
    "Знаєш, що я часто помічаю?\n\n"
    "Людина може роками думати, що їй просто бракує дисципліни.\n\n"
    "Але якщо вона постійно починає, кидає, знову починає — "
    "я б не поспішала звинувачувати її в цьому.\n\n"
    "Іноді сама система життя настільки перевантажена, "
    "що нова звичка просто не має куди в ній поміститися.\n\n"
    "Тому далі мені важливо зрозуміти не тільки <b>що ти хочеш змінити</b>, "
    "а й <b>як ти зараз живеш.</b>"
)

TEXT_BETWEEN_Q3_Q4 = (
    "Дякую ❤️\n\n"
    "Є одна річ, яку я хочу, щоб ти зараз помітила.\n\n"
    "Те, що забирає найбільше сил, не завжди є тією проблемою, "
    "яку ми першою хочемо вирішити.\n\n"
    "Наприклад, людина може дуже хотіти схуднути, але при цьому щодня "
    "бути настільки виснаженою, що на будь-які зміни просто не залишається ресурсу.\n\n"
    "Або хотіти більше заробляти, але весь її день уже заповнений справами.\n\n"
    "Саме тому я дивлюся на відповіді не окремо, а в сукупності."
)

PRERESULT_TEXT = (
    "Дякую ❤️\n\n"
    "Знаєш, мені подобається саме цей момент — коли вже можна перестати "
    "дивитися на окремі симптоми й побачити всю картину вцілому.\n\n"
    "І я хочу, щоб ти не сприймала результат, який зараз отримаєш, "
    "як діагноз чи оцінку тебе з мого боку.\n\n"
    "Уяви, що ми зараз сидимо з тобою у затишній кав'ярні, і ти питаєш "
    "у мене з чого б я почала на твоєму місці… "
    "моя відповідь — просто щира порада подруги, але вибір завжди за тобою.\n\n"
    "Зараз покажу ❤️"
)
...
async def q7_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_video_note(
        ...
        "Дякую ❤️ Знаєш, мені подобається саме цей момент..."
    )
    ...
    # Текст результату + кнопка переходу
    await ctx.bot.send_message(
        chat_id,
        result["text"],
        parse_mode="HTML",
    ...
    # Фінальний текст (однаковий для всіх) — після натискання кнопки
    await ctx.bot.send_message(
        chat_id,
        FINAL_TEXT,
        parse_mode="HTML"
...
def main():
    ...
    # Хендлер для отримання file_id медіа (поза розмовою)
